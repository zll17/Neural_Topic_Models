...
model = GMMVAE(bow_size=len(vocab),h_dim=1024,z_dim=n_topic,n_comps=num_comps).to(device)
...
for tp,cv in zip(topics_text,cv_coherence_score):
    print('{}+++$+++cv:{}'.format(tp,cv))
    logger.write('{}+++$+++cv:{}\n'.format(str(tp),cv))
