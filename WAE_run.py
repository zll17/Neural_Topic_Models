...
parser.add_argument('--dist',type=str,default='dirichlet',help='The prior distribution of the latten variables')
...
model = WAE(bow_size=len(vocab),h_dim=hdim,z_dim=n_topic,n_clust=n_clust,nonlin='relu').to(device)
...
def train(model, data_loader, num_epochs):
    ...
    for epoch in range(num_epochs):
        for i, x in enumerate(data_loader):
            ...
            # Forward pass
            x = x.to(device)
            x_reconst, z_fake = model(x)
            ag = {'dirichlet_alpha':0.1}
            z_real = model.sample(dist=dist,batch_size=z_fake.shape[0],args=ag).to(device)

            # Compute reconstruction loss and kl divergence
            reconst_loss = F.binary_cross_entropy(x_reconst, x, reduction='sum')
            mmd = model.mmd_loss(z_fake,z_real,device=device,t=0.1)
            #kl_div = torch.tensor([- 0.5 * torch.sum(1 + log_var - mu.pow(2) - log_var.exp()) for mu,log_var in zip(mus,logvars)])

            # Backprop and optimize
            s = torch.sum(x).float()/x.shape[0]
            lamb = (5.0*s*torch.log(torch.tensor(1.0*len(vocab)))/torch.log(torch.tensor(2.0)))
            mmd = mmd * lamb
            loss = reconst_loss + mmd
            #loss = reconst_loss
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            ...
            if (i+1) % 100 == 0:
                print("Epoch[{}/{}], Step [{}/{}], Reconst Loss: {:.4f},MMD: {:.4f}" 
                       .format(epoch+1, num_epochs, i+1, len(data_loader), reconst_loss.item()/batch_size, mmd.item()/batch_size))

                logger.write("Epoch[{}/{}], Step [{}/{}], Reconst Loss: {:.4f}, MMD: {:.4f}\n".format(epoch+1, num_epochs, i+1, len(data_loader), reconst_loss.item()/batch_size, mmd.item()/batch_size))
                rec_loss.append(reconst_loss.item()/batch_size)
                mmd_loss.append(mmd.item()/batch_size)
    ...
    logger.write('='*60+'\n\n')
    return rec_loss,mmd_loss,cv_lst,w2v_lst,td_lst


rec_loss,mmd_loss,cv_lst,w2v_lst,td_lst = train(model ,train_loader, num_epochs=num_epochs)
logger.write('#rec_loss: {}\n'.format(rec_loss))
logger.write('#mmd_div: {}\n'.format(mmd_loss))
logger.write('#cv_lst: {}\n'.format(cv_lst))
...
plt.subplot(2,2,2)
plt.title('MMD for {} {} topics'.format(taskname,n_topic))
plt.plot(list(range(len(mmd_loss))),mmd_loss)
